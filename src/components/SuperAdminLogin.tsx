    ...
    <div className="relative rounded-[25px] border border-white/10 bg-[#0a0c11] p-6 sm:p-8">
      ...
      <form onSubmit={submit} className="mt-7 space-y-3">
        ...
        <div className="mt-4 flex flex-col gap-3">
          <button disabled={loading} className="flex w-full items-center justify-center gap-3 rounded-2xl bg-[#dceeff] px-5 py-4 text-[12px] font-[900] text-[#08213a] transition hover:bg-white active:scale-[0.98] disabled:opacity-60">{loading ? "กำลังยืนยันตัวตน" : "เข้าสู่ศูนย์ควบคุม"}{loading ? <LoaderCircle className="size-4 animate-spin" /> : <ArrowRight className="size-4" />}</button>
          <a href="/" className="flex w-full items-center justify-center gap-2 rounded-2xl bg-white/5 px-5 py-3 text-[11px] font-[900] text-white/50 transition hover:bg-white/10 hover:text-white active:scale-[0.98]">กลับสู่หน้าแรก</a>
        </div>
      </form>
