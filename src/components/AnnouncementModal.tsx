...
export function AnnouncementModal({ data }: { data: AnnouncementData }) {
  ...
  useEffect(() => {
    ...
    const mutedUntil = localStorage.getItem("mute_announcement");
  ...
  function handleClose() {
    if (hideForHour) {
      ...
      localStorage.setItem("mute_announcement", oneHourFromNow.toString());
    }
    setIsOpen(false);
  }
          ...
          >
            ...
            <div className="flex items-center justify-between p-5 border-b border-white/10 bg-white/5 relative z-10 shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-[16px] font-[900] text-white tracking-wide">ประกาศจากระบบ</h2>
                </div>
              </div>
              <button 
                onClick={handleClose}
            ...
            <div className="overflow-y-auto custom-scrollbar relative z-10">
              {data.imageUrl && (
                <div className="w-full">
                  <img src={data.imageUrl} alt="Announcement" className="w-full h-auto object-cover max-h-[300px]" />
                </div>
              )}
              <div className="p-6">
                <h3 className="text-[18px] font-[900] text-white mb-3 leading-snug">{data.title}</h3>
                <div className="text-[13px] leading-relaxed text-[#a0b2c3] whitespace-pre-wrap">
                  {data.content}
                </div>
              </div>
            </div>
            ...
            <div className="p-5 border-t border-white/10 bg-black/40 relative z-10 shrink-0 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <label className="flex items-center gap-2 cursor-pointer group">
                <div className={`grid size-5 place-items-center rounded-md border transition-colors ${hideForHour ? "bg-[#0084ff] border-[#0084ff]" : "bg-black/50 border-white/20 group-hover:border-white/40"}`}>
                  {hideForHour && <X className="size-3 text-white hidden" />} {/* Just for sizing trick if needed, or use a checkmark */}
