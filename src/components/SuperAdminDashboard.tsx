...
export function SuperAdminDashboard({ gangs }: { gangs: SuperAdminGang[] }) {
  ...
  async function deleteGang(gang: SuperAdminGang) {
    if (!window.confirm(`⚠️ อันตราย! ยืนยันการลบแก๊ง "${gang.pageTitle}" ทิ้งถาวรหรือไม่? \nข้อมูลทุกอย่างของแก๊งนี้จะหายไปและไม่สามารถกู้คืนได้`)) return;
    const prompt = window.prompt(`พิมพ์คำว่า "${gang.subdomain}" เพื่อยืนยันการลบ:`);
    if (prompt !== gang.subdomain) {
      alert("พิมพ์ไม่ถูกต้อง ยกเลิกการลบ");
      return;
    }
    setResetting(gang.id); setError("");
    try {
