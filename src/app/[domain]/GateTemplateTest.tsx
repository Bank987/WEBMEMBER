...
export default function GateTemplateTest({ settings }: { settings: Settings }) {
    ...
    >
      {settings.backgroundImageUrl && (
        <div className="absolute inset-0 z-[1] pointer-events-none">
          ...
          <div className="absolute inset-0 z-[2] bg-gradient-to-b from-black/80 to-black" />
        </div>
      )}
      ...
      <div className="relative z-[10] text-center px-4 max-w-4xl mx-auto flex flex-col items-center w-full">
        ...
        >
          ...
          <div className="flex items-center justify-center w-full max-w-[400px] mt-[32px] opacity-60">
            ...
            <span className="px-[12px] text-[10px] font-bold tracking-[0.3em] text-white/50 uppercase">
              Partner
            </span>
            <div className="flex-1 h-[1px] bg-gradient-to-l from-transparent to-white/20"></div>
          </div>

          {/* Partner Buttons (Max 3) */}
          <div className="flex flex-row justify-center gap-[16px] mt-[16px] w-full max-w-[600px]">
             {[
               { name: "KODOMO" },
               { name: "GODYOUKNOW" },
               { name: "THREETHOUSAND" }
             ].slice(0, 3).map((partner, idx) => (
                  <a href="#" key={idx} className="w-full max-w-[180px] group">
                    <motion.div
