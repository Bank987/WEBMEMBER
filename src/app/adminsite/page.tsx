...
import { getAllVipKeys, getSuggestions } from "@/lib/db";
import { SuperAdminSuggestions } from "@/components/SuperAdminSuggestions";
...
export default async function SuperAdminPage() {
  ...
  return <main className="relative min-h-screen overflow-hidden bg-[#080a0e] px-5 py-8 text-white md:px-10 md:py-12">
    ...
    <div className="relative mx-auto max-w-[1240px]">
      ...
      <SuperAdminSuggestions initialSuggestions={JSON.parse(JSON.stringify(suggestions))} />
      <SuperAdminDashboard gangs={gangs} />
      <SuperAdminVipKeys initialKeys={JSON.parse(JSON.stringify(vipKeys))} />
    </div>
  </main>;
}
