...
// 1. Remove the wrongly injected code
const wrongInjectStart = '{/* Floating Stats - Top Right */}';
const wrongInjectEnd = '        </motion.div>\n\n        {/* Section Header */}';

const regex = new RegExp(
  wrongInjectStart.replace(/[.*+?^\${}()|[\]\\]/g, '\\$&') + 
  '(.|\\n)*?' + 
  wrongInjectEnd.replace(/[.*+?^\${}()|[\]\\]/g, '\\$&'),
  'g'
);

content = content.replace(regex, '{/* Section Header */}');

// 2. Inject it at the right place (the FIRST Section Header)
const rightAnchor = '{/* Section Header */}\n        <motion.div \n          initial={{ opacity: 0, y: -20 }}';
...
content = content.replace(rightAnchor, topStatsStr);
