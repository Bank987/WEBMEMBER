...
export default function GateClient({ settings }: { settings: Settings }) {
  const theme = getGangTheme(settings.theme || "default", settings.customAccentColor);

  useEffect(() => {
    // Apply theme variables directly
    document.documentElement.style.setProperty('--gang-accent', theme.accent);
    document.documentElement.style.setProperty('--gang-glow', theme.glow);
  }, [theme]);
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <div className="relative z-[10] px-8 max-w-6xl mx-auto flex flex-col-reverse md:flex-row items-center justify-between gap-12 md:gap-24 w-full mb-20 md:mb-0">
        ...
        <div className="flex flex-col items-center md:items-start w-full md:w-1/2">
          ...
          <motion.div
            ...
            className="flex flex-col items-center md:items-start justify-center md:justify-start gap-[18px] mt-[24px] w-full"
          >
            <div className={
            ...
            {settings.partnersEnabled && settings.partners && settings.partners.length > 0 && (
              <>
                <div className="flex items-center justify-center md:justify-start w-full max-w-[400px] mt-[32px] opacity-60">
                  <div className="flex-1 h-[1px] bg-gradient-to-r from-transparent to-white/20 md:hidden"></div>
                  <span className="px-[12px] md:pl-0 md:pr-[12px] text-[12px] font-[800] tracking-[0.3em] text-white/50 uppercase">
                    Partner
                  </span>
                  <div className="flex-1 h-[1px] bg-gradient-to-l from-transparent to-white/20 md:bg-gradient-to-r md:from-white/20 md:to-transparent"></div>
                </div>

                <div className="flex flex-row flex-wrap justify-center md:justify-start gap-[16px] mt-[16px] w-full max-w-[600px]">
        ...
        <div className="flex flex-col items-center md:items-end text-center md:text-right w-full md:w-1/2">
          {settings.logoUrl && (
            <div 
              className="relative w-[340px] h-[340px] mb-[40px] mt-[24px] flex items-center justify-center mx-auto md:ml-auto md:mr-0"
          ...
          <div className="mt-8 mb-12 flex flex-col items-center md:items-end w-full">
            <motion.h2 
...
fs.writeFileSync('src/app/[domain]/GateClient.tsx', fullFile);
console.log('Successfully applied full GateClient.tsx');
