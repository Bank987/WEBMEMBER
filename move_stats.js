...
// 1. Remove the stats bar from below the search box
const oldStatsRegex = /\{\/\* Stats Bar - Refined Capsules \*\/\}(.|\n)*?(?=\{\/\* Directory Sections \*\/\})/g;
content = content.replace(oldStatsRegex, '');

// 2. Inject it at the top right
const headerAnchor = '{/* Section Header */}';
...
content = content.replace(headerAnchor, topStatsStr + headerAnchor);

fs.writeFileSync('src/app/[domain]/members/MembersClient.tsx', content);
console.log('Moved stats to top right');
